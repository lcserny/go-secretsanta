...
func (c *matchesController) findMatch(writer http.ResponseWriter, request *http.Request) {
    ...
    if (!found) {
        tmpl, err := template.ParseFiles("assets/invalid.html")
        if err != nil {
            http.Error(writer, err.Error(), http.StatusInternalServerError)
            return
        }
...
func (c *matchesController) clearMatches(writer http.ResponseWriter, request *http.Request) {
...
func InitMatchesController(router *mux.Router) {
	...
	router.HandleFunc(fmt.Sprintf("/%s", MATCHES_ROUTE), controller.generateLinks).Methods("POST")
	router.HandleFunc(fmt.Sprintf("/%s/{from}/{token}", MATCHES_ROUTE), controller.findMatch).Methods("GET")
	router.HandleFunc(fmt.Sprintf("/%s", MATCHES_ROUTE), controller.clearMatches).Methods("DELETE")
}
